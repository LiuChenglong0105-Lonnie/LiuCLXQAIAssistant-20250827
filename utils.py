...
def custom_paginate_and_render(blocks, prefix, render_func, page_size=10, enable_batch_summary=False, summary_type=None, ai_blocks=None):
    ...
    try:
        ...
        # 分页控件
        if total_pages > 1:
            # 生成唯一ID用于分页控件，解决key重复问题
            # 使用时间戳+前缀哈希的方式，确保即使相同prefix也能生成不同的唯一ID
            import hashlib
            import time
            import random
            ...
            # 组合前缀、时间戳和随机数生成唯一ID
            unique_string = f"{prefix}_{timestamp}_{random_num}"
            ...
            with st.container():
                ...
                with col1:
                    if st.button("上一页", key=f"{prefix}_prev_{unique_id}") and st.session_state[page_key] > 1:
                        st.session_state[page_key] -= 1
                        st.rerun()
                with col2:
                    ...
                    try:
                        st.session_state[page_key] = st.selectbox(
                            ...
                            key=f"{prefix}_page_select_{unique_id}"
                        )
                    except Exception as e:
                        print(f"页码选择出错: {e}")
                        st.session_state[page_key] = 1
                with col3:
                    if st.button("下一页", key=f"{prefix}_next_{unique_id}") and st.session_state[page_key] < total_pages:
                        st.session_state[page_key] += 1
                        st.rerun()
